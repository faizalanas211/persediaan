    ...
    public function collection(Collection $rows)
    {
        try {
            foreach ($rows as $index => $row) {
                ...
                if (!$namaBarang) {
                    throw new \Exception(
                        "Nama barang kosong di baris " . ($index + 1)
                    );
                }

                if ($stokFisik < 0) {
                    throw new \Exception(
                        "Stok fisik tidak valid di baris " . ($index + 1)
                    );
                }
                ...
                if (!$barang) {
                    throw new \Exception(
                        "Barang '{$namaBarang}' tidak ditemukan (baris " . ($index + 1) . ")"
                    );
                }
            ...
            throw $e; // ⬅️ PENTING: biar naik ke controller
        }
    }
